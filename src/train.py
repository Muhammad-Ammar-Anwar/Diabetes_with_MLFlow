...
def train(data_path,model_path,random_state,n_estimators,max_depth):
    ...
    with mlflow.start_run():
        ...
        # Try to log model to MLflow, skip if it fails (e.g., DagsHub doesn't support model logging)
        model_logged = False
        if tracking_url_type_store!='file':
            try:
                ...
                print(f"Warning: Model registration failed ({type(e1).__name__}), trying without registration...")
                try:
                    mlflow.sklearn.log_model(best_model, "model", signature=signature)
                    model_logged = True
                except Exception as e2:
                    print(f"Warning: Could not log model to MLflow ({type(e2).__name__}): {e2}")
                    print("Model will still be saved locally. Metrics and parameters have been logged successfully.")
        else:
            try:
                mlflow.sklearn.log_model(best_model, "model", signature=signature)
                model_logged = True
            except Exception as e:
                print(f"Warning: Could not log model to MLflow ({type(e).__name__}): {e}")
                print("Model will still be saved locally. Metrics and parameters have been logged successfully.")
